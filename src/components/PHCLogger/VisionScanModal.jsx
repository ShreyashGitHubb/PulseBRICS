        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-slate-400">•</span>
              <span className="text-[11px] font-mono px-2 py-0.2 rounded bg-blue-500/10 text-[#1A73E8] dark:text-[#8AB4F8] border border-[#1A73E8]/30">
          ...
          <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400 font-semibold">
            ...
            {customImageBase64 && (
              <button
                ...
                className="text-[11px] text-[#1A73E8] dark:text-[#8AB4F8] hover:underline flex items-center space-x-1 cursor-pointer"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {SAMPLE_IMAGES.map((sample, idx) => (
              ...
              >
                ...
                <div className="text-[10px] text-slate-500 dark:text-slate-400 truncate mt-0.5">{sample.description}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div
          ...
          className={`relative rounded-xl overflow-hidden border-2 transition-all p-4 text-center ${
            isDragOver 
              ...
              : 'border-dashed border-[#DADCE0] dark:border-[#3C4043] bg-[#F8F9FA] dark:bg-[#131314] hover:border-[#1A73E8]'
          }`}
        >
          <input
            type="file"
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-4">
            <div className="w-40 h-28 rounded-lg overflow-hidden border border-[#DADCE0] dark:border-[#3C4043] bg-black shrink-0 relative group">
              <img
                src={currentPreview}
            ...
            <div className="space-y-2 text-left">
              ...
              <div className="flex items-center space-x-2 pt-1">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-white dark:bg-[#28292A] hover:bg-[#F1F3F4] dark:hover:bg-[#35363A] text-slate-800 dark:text-slate-200 text-xs font-semibold border border-[#DADCE0] dark:border-[#3C4043] flex items-center space-x-1.5 transition-colors cursor-pointer"
                >
                  <UploadCloud className="w-3.5 h-3.5 text-[#1A73E8] dark:text-[#8AB4F8]" />
        ...
        {visionResult && (
          <div className="p-4 rounded-xl bg-[#F8F9FA] dark:bg-[#131314] border border-[#DADCE0] dark:border-[#3C4043] space-y-3 animate-fadeIn">
            ...
            <div className="space-y-1.5">
              {visionResult.detectedItems?.map((item, i) => (
                <div key={i} className="flex items-center justify-between text-xs p-2.5 bg-white dark:bg-[#1E1F20] rounded-lg border border-[#DADCE0] dark:border-[#3C4043]">
                  ...
                  <div className="flex items-center space-x-2">
                    <span className="font-mono text-[#1A73E8] dark:text-[#8AB4F8] font-bold">{item.detectedCount} {item.unit}</span>
                    <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${
                      item.status === 'CRITICAL_STOCKOUT' ? 'bg-[#EA4335]/15 text-[#EA4335] dark:text-[#F28B82]' : 'bg-[#F1F3F4] dark:bg-[#28292A] text-slate-700 dark:text-slate-300'
                    }`}>
                      {item.status}
                    </span>
                  </div>
            ...
            {visionResult.visualAnomalyDetected && (
              <div className="p-2.5 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 text-xs text-amber-800 dark:text-amber-200 flex items-start space-x-2">
